...
# --- Assign variables
  ...
  # Assign additional variables
    ...
    RM[, PREVTPAsum     := sumNA(start1tpa), by=PLT_CN                 ]

    # Final number of trees is current survivors plus new trees that cross the 5" threshold
    RM[, end1tpa        := SURVIVORTPA + INGROWTHTPA                   ]
    RM[, TPAsum         := sumNA(end1tpa), by=PLT_CN                   ]


# --- Subset for output?
  S3RM = RM[, .(PLT_CN, PREVTPAsum, TPAsum, PREVDIAmean, DIAmean)]
...
#     fia.con = db.open(dbsettings)
  ...
  tic() # ~20 sec
  TREE3 = TCPb[,.(CN, DIA, HT, STOCKING, SPCD, TPA_UNADJ, AGENTCD, DSTRBCD1, STDAGE, MICROPLOT_LOC, SISP, CARBON_AG, CARBON_BG, PFT)]
  toc()
...
#  
#   TCP = merge(TREE3, TCPb, all.x=TRUE, all.y=TRUE, by="PREV_TRE_CN")
...
write.csv(F3, file = "/Work/Research/Macrosystems/FIA Phase Space/Data/ESMrawRKv02.csv") #write to local drive
# write.csv(F3, file = "~/rykelly/PhaseSpace/ESMrawRKv01.csv") #write to local drive
