...
# Find state codes
    ...
    query = 'SELECT statecd FROM plot WHERE lon>-95'
    plots = db.query(query, con=fia.con)
    states = sort(unique(plots$statecd))
      states = states[states < 60]
...
timer=Sys.time()
progress.file = '~/Desktop/progress.txt'
writeLines(c(""), progress.file)
# Foreach
...
{
  sink(progress.file, append=T)
    cat( "Querying GRM for state ", state.names[i], "...\n", sep="")
...
# 			TPAMORT_UNADJ, ANN_NET_GROWTH, REMOVALS, MORTALITY, DIA_BEGIN, DIA_END, COMPONENT, DIA_BEGIN_RECALC)) 
    ...
    fia.con = db.open(dbsettings)
		...
		query = paste('SELECT 
		          ...
		          ann_net_growth, removals, mortality, dia_begin, dia_end, component, dia_begin_recalc
		        FROM tree_grm_estn WHERE statecd=', states[i], 
...
# 	RM1  <-  subset(RM)    
  ...
  RM2  <-  subset(RM, cut==0 & start>0, select=c(PLT_CN, PREVTPAsum, TPAsum, PREVDIAmean, DIAmean))
...
{
  sink(progress.file, append=T)
    cat( "Querying S3 for ", state.names[i], "...\n", sep="")
   ...
   # Open connection to database
    fia.con = db.open(dbsettings)
		...
		query = paste('SELECT 
		        ...
		        FROM plot WHERE statecd=', states[i], 'AND lon>-95')
...
#             setnames(PLOT, "CN", "PLT_CN")
	...
	{	
			query = paste('SELECT 
		        ...
		        FROM tree WHERE statecd=', states[i])
...
# 	TREE   <-   subset(as.data.table(read.csv(treefiles[i], header = TRUE, sep = ",", quote="\"", dec=".")), select=c(CN, PREV_TRE_CN, PLT_CN, INVYR,CONDID, DIA, TPA_UNADJ, HT, AGENTCD, DAMTYP1, DECAYCD, SPCD, STOCKING, STATUSCD, STANDING_DEAD_CD, SPGRPCD, CR, CCLCD, RECONCILECD, DIACHECK, DIAHTCD, PREVDIA, PREV_STATUS_CD, P2A_GRM_FLG, CARBON_AG, CARBON_BG, PLOT, TREE))
			...
			query = paste('SELECT 
		        ...
		        FROM cond WHERE statecd=', states[i])
